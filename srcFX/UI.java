...
public class UI extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        movies.addListener((ListChangeListener) change -> list.getChildren().add(createEntry(movies.get(movies.size() - 1))));
        movies.addListener(movieList::updateOriginal);


        BorderPane bottom = new BorderPane();
        root.setBottom(bottom);
        Text t = new Text("Number of entries: " + entries);
        bottom.setLeft(t);
        btn = new Button("+");
        btn.setOnAction(e ->
...
*/
    ...
    private void addMovie(Stage primaryStage, ObservableList<MovieFX> movieList) {
        ...
        //GridPane.setFillWidth(lYear, true);     //
        //lYear.setMaxWidth(Double.MAX_VALUE);                //
    ...
    private HBox createEntry(MovieFX movie) {
        entries++;
        //one entry
        HBox entry = new HBox();
        entry.setSpacing(30);
